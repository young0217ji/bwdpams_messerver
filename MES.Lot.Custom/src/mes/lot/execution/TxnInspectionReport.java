 ...
 * @see
 */
public class TxnInspectionReport implements ObjectExecuteService {
	/**
	 * 검사성적서 Report를 수정하는 트랜잭션 실행
	 * 
	 * @param recvDoc
	 * @return Object
	 * 
	...
	public Object execute(Document recvDoc) {
		...
		for (int i = 0; i < masterData.size(); i++) {
			...
			DY_INSPECTIONREPORT dataInfo = new DY_INSPECTIONREPORT();
			// Key Value Setting
			...
			if ("C".equals(dataMap.get("_ROWSTATUS")) == false) {
				dataInfo = (DY_INSPECTIONREPORT) dataInfo.excuteDML(SqlConstant.DML_SELECTFORUPDATE);
			}

			// Data Value Setting
			dataInfo.setHC_INSIDE_DIA(dataMap.get("HC_INSIDE_DIA"));
			dataInfo.setHC_SIDTH(dataMap.get("HC_SIDTH"));
			dataInfo.setKC_INSIDE_DIA(dataMap.get("KC_INSIDE_DIA"));
			dataInfo.setKC_SIDTH(dataMap.get("KC_SIDTH"));
