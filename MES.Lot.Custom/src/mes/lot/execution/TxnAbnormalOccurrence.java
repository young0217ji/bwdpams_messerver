...
public class TxnAbnormalOccurrence implements ObjectExecuteService {

	/**
	 * 이상발생을 등록, 수정하는 트랜젝션 실행
	 * 
	 * @param recvDoc
	 * @return Object
	 */
	...
	public Object execute(Document recvDoc) 
	{
		...
		for(int i = 0; i< masterData.size(); i++)
		{
			...
			String dataKey = DateUtil.getCurrentEventTimeKey();
			...
			if(_rowStatus.equalsIgnoreCase("C"))
			{
				//Create
				if(i == 0)
				{
					DYInterfaceManager.callSMSInputService(plantID, txnInfo.getTxnUser(), description, "TxnAbnormalOccurrence");
					...
					dy_abnormalOccurrence.setCREATETIME(txnInfo.getEventTime());
					dy_abnormalOccurrence.setCREATEUSERID(txnInfo.getTxnUser());
					
					dy_abnormalOccurrence.excuteDML(SqlConstant.DML_INSERT);
				}
				...
				{
					//Detail 등록..
					// DY_ABNORMALOCCURRENCEDETAIL
					// Key Value Setting
					dy_abnormalOccurrenceDetail.setKeyPLANTID(plantID);
					dy_abnormalOccurrenceDetail.setKeyDATAKEY(dataKey);
					dy_abnormalOccurrenceDetail.setKeyITEMID(itemID);

					// Data Value Setting
					dy_abnormalOccurrenceDetail.setDESCRIPTION(description);
					dy_abnormalOccurrenceDetail.setCREATETIME(txnInfo.getEventTime());
					dy_abnormalOccurrenceDetail.setCREATEUSERID(txnInfo.getTxnUser());
					
					dy_abnormalOccurrenceDetail.excuteDML(SqlConstant.DML_INSERT); 
					history.addHistory(dy_abnormalOccurrenceDetail, txnInfo, SqlConstant.DML_INSERT);
				}
			}
			
			if(_rowStatus.equalsIgnoreCase("U"))
			{
				//Update Main
				// DY_ABNORMALOCCURRENCE
				// Key Value Setting
				dy_abnormalOccurrence.setKeyPLANTID(plantID);
				dy_abnormalOccurrence.setKeyDATAKEY(dataMap.get("DATAKEY"));
        ...
        history.addHistory(dy_abnormalOccurrence, txnInfo, SqlConstant.DML_INSERT);
