 ...
 * @see
 */
public class TxnTrackOutCancel implements ObjectExecuteService
{
	/**
	 * Durable ID 기준으로 Multi Lot에 대한 공정종료를 취소하는 트랜잭션 실행
	 * 
	 * @param recvDoc
	 * @return Object
	 * 
	...
	public Object execute(Document recvDoc)
	{
		...
		if ( dataList.size() > 0 ) 
		{
			...
			durableService.moveIn(durableInfo, txnInfo);
			...
			for ( int i = 0; i < listLotInfo.size(); i++ )
			{
				...
				operationService.processEndCancel(lotInfo.getKeyLOTID(), processID, processSequence, routeRelationSequence, txnInfo);
			}
		}
