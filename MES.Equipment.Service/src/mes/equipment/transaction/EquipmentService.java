...
public class EquipmentService
{
    ...
    */
	public void pmEnd(HashMap<String, String> dataMap, TxnInfo txnInfo) {
		...
		DY_EQUIPMENTPMCODEMAP oPMCodeMap = new DY_EQUIPMENTPMCODEMAP();
		oPMCodeMap.setKeyPLANTID(equipmentPMScheduleInfo.getKeyPLANTID());
		oPMCodeMap.setKeyEQUIPMENTID(equipmentPMScheduleInfo.getKeyEQUIPMENTID());
		oPMCodeMap.setKeyPMCODEID(equipmentPMScheduleInfo.getREFERENCEVALUE());
		try {
			oPMCodeMap = (DY_EQUIPMENTPMCODEMAP) oPMCodeMap.excuteDML(SqlConstant.DML_SELECTFORUPDATE);
			if ( "LASTPM".equalsIgnoreCase(oPMCodeMap.getSCHEDULETYPE()) ) {
				// LASTPM Type인 경우 마지막 스케줄 코드 저장
				// 이력을 저장하지 않음!
				oPMCodeMap.setSCHEDULETVALUE(equipmentPMScheduleInfo.getKeyPMSCHEDULEID());
				oPMCodeMap.excuteDML(SqlConstant.DML_UPDATE);
			}
		}
